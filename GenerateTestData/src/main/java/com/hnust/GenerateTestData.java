...
//        System.out.println(localDateTime.format(new DateTimeFormatter("yyyy-MM-dd HH:mm:ss")));
    ...
    public void generateData() throws IOException {
        ...
        BufferedReader userInfoReader = new BufferedReader(new FileReader("userInfo.txt"));
        BufferedReader urlReader = new BufferedReader(new FileReader("urls.txt"));
        ...
        LocalDateTime dateTime = LocalDateTime.of(2020, 4, 4, 0, 0);
        ...
        for (int i = 0; i < 180; i++) {
            ...
            String path = "D:\\log-data\\"+date+".txt";
...
//            int count = 0,totalCount = 10;
    ...
    public LogData randomEvent(long baseTime){
        ...
        if (url.contains("appointment")){
            //预约事件
            event = appointmentEventGenerate(userInfo,baseTime);
        }
        ...
        if (url.contains("deal")){
            //成交事件
            event = dealEventGenerate(userInfo,baseTime);
        }
    ...
    //生成预约事件
    public Event appointmentEventGenerate(UserInfo userInfo,long time){
    ...
    //生成成交事件
    public Event dealEventGenerate(UserInfo userInfo,long time){
        Event event = new Event();
    ...
    public Event favoritesEventGenerate(UserInfo userInfo,long time){
        ...
        favoritesEvent.setHourse_id(hourse.getHourseId());
        ...
        favoritesEvent.setAdd_time(time+"");
    ...
    public Event praiseEventGenerate(UserInfo userInfo,long time){
        ...
        praiseEvent.setTarget_id((72387+random.nextInt(789793232))+"");
        praiseEvent.setType((1+random.nextInt(2))+"");
        praiseEvent.setAdd_time(time+"");
    ...
    //生成搜索事件
    public Event searchEventGenerate(UserInfo userInfo,long time,String url){
        Event event = new Event();
    ...
    public void generateUserInfo() throws IOException, ClassNotFoundException, SQLException {
        ...
        Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/movie?useSSL=false&allowPublicKeyRetrieval=true&serverTimezone=UTC","root","root");
        ...
        for (int i = 0; i < 3000; i++) {
            ...
            String sql = "SELECT mername,Lng,Lat FROM region LIMIT "+random.nextInt(3700)+",1;";
            ...
            if (rs.next()){
                ...
                if (split.length > 3){
                    userInfo.setArea(split[1]+","+split[2]);
                }else{
                    continue;
                }
    ...
    public void generateHourse() throws Exception {
        ...
        for (int i = 0; i < 5000; i++) {
            ...
            String sql = "SELECT mername,Lng,Lat FROM region LIMIT "+random.nextInt(3700)+",1;";
            String sql2 = "SELECT mername,Lng,Lat FROM region WHERE mername LIKE '%海南%' LIMIT "+random.nextInt(25)+",1;";
